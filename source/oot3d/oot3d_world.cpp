...
WorldBuildingError Oot3dWorld::BuildLocationTable()
{
    ...
    for (auto locationIt = locationDataTree.Begin(); locationIt != locationDataTree.End(); locationIt++)
    {
        ...
        // Set the location as a proper item location as long as it isn't a hint location, drop, or hardcoded event
        if (!IsAnyOf(type, Oot3dLocationType::HintStone, Oot3dLocationType::OtherHint, Oot3dLocationType::Drop, Oot3dLocationType::Event))
        {
            locations[locationId]->SetAsItemLocation();
        }
    }
    return WorldBuildingError::NONE;
}
...
WorldBuildingError Oot3dWorld::BuildItemPools()
{
    ...
    BUILD_ERROR_CHECK(GenerateOot3dStartingInventory());

    // Take away any starting inventory items from the item pool
